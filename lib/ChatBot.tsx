...
export class ChatBot {
  ...
  constructor(_userMessage: string | null) {
    this.assistant_id = process.env.OPENAI_ASSISTANT_ID!;
    this.threadId = null;
  ...
  private async createChatBot(): Promise<void> {
    ...
    try {
      this.TPXChatBot = await this.openai.beta.assistants.retrieve(
        this.assistant_id,
      );
  ...
  public async startThreadWithMessage(
    _userMessage: string,
  ...
  private parseResponseToHTML(response: string): string {
    ...
    lines.forEach((line, index) => {
      ...
      // Check for unordered list items (starting with "-" or "*")
        ...
        //   : (html += `<p style="margin-bottom: 10px;">${trimmedLine}</p>`);
        if (index === lines.length - 1) html += `<p>${trimmedLine}</p>`;
        else html += `<p style="margin-bottom: 10px;">${trimmedLine}</p>`;
      }
    });
