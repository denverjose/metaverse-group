...
export async function startChatAction() {
  ...
  }); // Store the chatbot with null threadID initially
...
export async function sendMessageAction(chatId: string, message: string) {
  ...
  await db.run(
    `INSERT INTO Message (id, chatId, sender, content, createdAt) VALUES (?, ?, ?, ?, datetime('now'))`,
  ...
  try {
    ...
    const [response, threadID] = await chatBot.startThreadWithMessage(
      sanitizedMessage,
      initialThreadID,
    );
    ...
    if (initialThreadID !== threadID) {
      await db.run(
        `UPDATE Chat SET threadID = ?, updatedAt = datetime('now') WHERE id = ?`,
        [threadID, chatId],
      );
